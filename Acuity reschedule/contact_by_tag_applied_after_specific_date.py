...
class ContactByTagAppliedAfterSpecificDate():
    def __init__(self, tagIdList, afterDate):
        ...
        # returnFields = ['Contact.Id', 'ContactId', 'Contact.FirstName', 'Contact.LastName', 'Contact.Email', 'DateCreated']  # removed Infusionsoft changed 'DateCreated field'
        returnFields = ['Contact.Id', 'ContactId', 'Contact.FirstName', 'Contact.LastName', 'Contact.Email']

        #query = {'GroupId': tagIdList[0], 'DateCreated': '~>~ 2018-10-19'}  # 1480 All Paying Members # TODO pass in the date to filter the when the tag was applied
        query = {'GroupId': tagIdList[0], 'DateCreated': '~>~ ' + str(afterDate.year) + '-' + str(afterDate.month) + '-' + str(afterDate.day)}  # 1480 All Paying Members # TODO pass in the date to filter the when the tag was applied
        limit = 1000
        page = 0
